...
export function AppPage() {
  ...
  function handleCSVUpdate(file?: File | null) {
    ...
    reader.onload = e => {
      ...
      try {
        ...
        setSelectedFileName(undefined);
        toast.error('Error wail parsing CSV');
      }
    };
  ...
  async function handleSubmit() {
    ...
    if (isUsingNativeToken === false && !tokenAddressInputValue)
      return toast.error('Enter token address');

    const tokenAddress =
      isUsingNativeToken === false ? tokenAddressInputValue : NULL_ADDRESS;

    if (!csvData) return toast.error('Upload a valid csv file');
    ...
    if (startDateUnix <= new Date().getTime() / 1000) {
      toast.error(`Start time is already expired`);
      setLoadingMessage(undefined);
      setIsLoading(false);
      return;
    }
    ...
    if (isUsingNativeToken === false) {
      if ((await tokenContract.balanceOf(address)).lt(totalAmount)) {
        toast.error("This wallet doesn't have enough balance");
        setLoadingMessage(undefined);
        setIsLoading(false);
        return;
      }
    ...
    try {
      if (isPayingToken === true) {
        ...
        var validatorResponse = await fetch(
          `https://ij340ixcx1.execute-api.eu-central-1.amazonaws.com/api/getSignature?user=${address}&chainId=${chain?.id}`,
        );

        const { discount, signature, validity } =
        ...
        validatorResponse = await fetch(
          `https://ij340ixcx1.execute-api.eu-central-1.amazonaws.com/api/getSignature?user=${address}&chainId=${chain?.id}`,
        );
        const { discount, signature, validity } =
          await validatorResponse.json();
        const discountAmount = (
          (parseInt(feeValue) * parseInt(discount)) /
          100
        ).toString();
        const options = {
          value:
            isUsingNativeToken === true
              ? totalAmount.add(feeValue).sub(discountAmount)
              : feeValue.sub(discountAmount),
        };
        const transaction = await contract.createNewAirdrop(
          isPayingToken,
          tokenAddress,
      ...
      if (code !== undefined && Object.keys(ErrorCode).includes(code)) {
        ...
        toast.error('Some error occured! Check console for mor details.');
      }
      console.error(e);
    }
  }
