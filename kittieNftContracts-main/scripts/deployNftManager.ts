...
async function main() {
    ...
    await test_util.verify(kittieNft1.address, contractName, [1, 100, parseEther("0.03"), 10000, "Bastepaat's NFT Collection", "Bastepaat", "https://api.kitties.com/kitties/"])
    ...
    await test_util.sleep(60);
    await test_util.updateABI(contractName)
    await test_util.verify(kittieNft2.address, contractName, [2, 60, parseEther("0.015"), 20000, "Mafdet's NFT Collection", "Mafdet", "https://api.kitties.com/kitties/"])
    ...
    kittieNft3 = await contractFactory.deploy(
        ...
        20000, // _maxSupply
        "Shumose's NFT Collection", // _name
        "Shumose", // _symbol
        "https://api.kitties.com/kitties/", // _initBaseURI
    ) as KittieNft;
    ...
    await test_util.sleep(60);
    await test_util.updateABI(contractName)
    await test_util.verify(kittieNft2.address, contractName, [3, 30, parseEther("0.005"), 30000, "Shumose's NFT Collection", "Shumose", "https://api.kitties.com/kitties/"])
